...
def get_ping_command(ip: str, timeout_ms: int):
    ...
    # Unix-like: -c 1 send 1 ping, -W secs (Linux). Convert ms -> secs (min 1s)
    secs = max(1, int((timeout_ms + 999) // 1000))
    return ["ping", "-c", "1", "-W", str(secs), str(ip)]


def ping_once(ip: str, timeout_ms: int) -> bool:
    """Ping a single IP once. Return True if reachable, False otherwise."""
...
def sweep_network(network_cidr: str, threads: int, timeout_ms: int, quiet: bool = False):
    ...
    logger.info("Starting sweep of %s (%d hosts) with %d threads", network_cidr, len(hosts), threads)

    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        futures = {executor.submit(ping_once, ip, timeout_ms): ip for ip in hosts}
...
def parse_args():
    ...
    p.add_argument("--threads", type=int, default=32, help="Number of parallel threads (default: 32)")
    p.add_argument("--timeout", type=int, default=300, help="Timeout in milliseconds (default: 300)")
...
def main():
    ...
    total = len(results)
    logger.info("Scan finished: %d/%d hosts up (%.1fs)", up_count, total, elapsed)
